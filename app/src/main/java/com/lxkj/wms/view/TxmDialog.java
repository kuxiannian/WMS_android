...
import com.lxkj.wms.ui.fragment.hwdj.adapter.TxmAdapter;
 ...
 */
public class TxmDialog extends Dialog {

    private Context context;      // 上下文
    RecyclerView recyclerView;

    public TxmDialog(Context context, List<String> barCodes, boolean isCan) {
        super(context, R.style.Theme_dialog); //dialog的样式
        this.context = context;
        Window window = getWindow();
        window.setGravity(Gravity.CENTER); // 此处可以设置dialog显示的位置为居中
        setContentView(R.layout.dialog_txm);
        ...
        recyclerView.setAdapter(new TxmAdapter(context,barCodes));
    }
